...
class KhrogAzkarPage extends StatefulWidget {
  KhrogAzkarPage({Key? key, required this.categoryName, required this.id}) : super(key: key);
  final String categoryName;
  final int id;
  var now=DateTime.now();
  @override
  State<KhrogAzkarPage> createState() => _KhrogAzkarPageState();
}

class _KhrogAzkarPageState extends State<KhrogAzkarPage> {
  late SharedPreferences prefs;
  late TimeOfDay _timeOfDay2=const TimeOfDay(hour: 7, minute: 00);
  ...
  Future<void> initializePreferences() async {
    ...
    setState(() {
      ...
      _timeOfDay2= getTimeFromPreferences() ?? const TimeOfDay(hour: 7, minute: 00);
      isSwitched2 = prefs.getBool('Azkar switcher${widget.id}') ?? false;
      //}
    });
  ...
  String getTimeDisplayString() {
    return _timeOfDay2.format(context).toString();
  }
  void saveAzkarSwitcherToSharedPreferences()  {
    // await prefs.setBool('Azan switcher', isAzanSwitched);

    prefs.setBool('Azkar switcher${widget.id}', isSwitched2);
  ...
  Future<void> saveTimeToPreferences() async{
    ...
    prefs.setInt('notificationHour${widget.id}', _timeOfDay2.hour);
    prefs.setInt('notificationMinute${widget.id}', _timeOfDay2.minute);
  ...
  Future<void> scheduleNotification() async {
    ...
    var selectedTime =DateTime(now.year,now.month,now.day,_timeOfDay2.hour,_timeOfDay2.minute);
