...
import 'package:azkar_muslims_app/services/boxes.dart';
import 'package:azkar_muslims_app/models/azkar_model.dart';
...
class RoqiaPage extends StatefulWidget {
   RoqiaPage({required this.categoryName, required this.id, });
   // final int hour;
   // final int minute;
  ...
  @override
  State<RoqiaPage> createState() => _RoqiaPageState();
}

class _RoqiaPageState extends State<RoqiaPage> {
  // List<AzkarModel> azkarScheduleList=[
  //   AzkarModel(isSwitched: false, timeOfDay: TimeOfDay(hour: 7, minute: 00), ),
  //   AzkarModel(isSwitched: false, timeOfDay: TimeOfDay(hour: 7, minute: 00), ),
  //   AzkarModel(isSwitched: false,timeOfDay: TimeOfDay(hour: 7, minute: 00), ),
  //   AzkarModel(isSwitched: false, timeOfDay: TimeOfDay(hour: 7, minute: 00), ),
  //   AzkarModel(isSwitched: false, timeOfDay: TimeOfDay(hour: 7, minute: 00), ),
  ...
  late TimeOfDay _timeOfDay8=TimeOfDay(hour: 7, minute: 00);
  ...
  Future<void> initializePreferences() async {
    ...
    setState(() {
      ...
      _timeOfDay8= getTimeFromPreferences() ?? TimeOfDay(hour: 7, minute: 00);
        isSwitched8 = prefs.getBool('Azkar switcher${widget.id}') ?? false;
      //}
    });
  ...
  void _showTimePicker() async {
      ...
      final selectedTime= await showTimePicker(
        context: context,
        initialTime: _timeOfDay8,
      );

      if (selectedTime != null) {
        setState(() {
          _timeOfDay8= selectedTime;
        });
  ...
  String getTimeDisplayString() {
    return _timeOfDay8.format(context).toString();
  }
  void saveAzkarSwitcherToSharedPreferences()  {
    // await prefs.setBool('Azan switcher', isAzanSwitched);

      prefs.setBool('Azkar switcher${widget.id}', isSwitched8);
  ...
  Future<void> saveTimeToPreferences() async{
    //for (int i=0;i<azkarScheduleList.length;i++){
        ...
        prefs.setInt('notificationHour${widget.id}', _timeOfDay8.hour);
        prefs.setInt('notificationMinute${widget.id}', _timeOfDay8.minute);
   ...
   Future<void> scheduleNotification() async {
    final now=DateTime.now();
     var selectedTime =DateTime(now.year,now.month,now.day,_timeOfDay8.hour,_timeOfDay8.minute);
