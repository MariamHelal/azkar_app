...
class EatingAzkarPage extends StatefulWidget {
  EatingAzkarPage({Key? key, required this.categoryName, required this.id}) : super(key: key);
  final String categoryName;
  final int id;
  var now=DateTime.now();
  @override
  State<EatingAzkarPage> createState() => _EatingAzkarPageState();
}

class _EatingAzkarPageState extends State<EatingAzkarPage> {

  late SharedPreferences prefs;
  late TimeOfDay _timeOfDay7=const TimeOfDay(hour: 7, minute: 00);
  ...
  Future<void> initializePreferences() async {
    ...
    setState(() {
      ...
      _timeOfDay7= getTimeFromPreferences() ?? const TimeOfDay(hour: 7, minute: 00);
      isSwitched7 = prefs.getBool('Azkar switcher${widget.id}') ?? false;
      //}
    });
  ...
  void _showTimePicker() async {
    ...
    final selectedTime= await showTimePicker(
      context: context,
      initialTime: _timeOfDay7,
    );

    if (selectedTime != null) {
      setState(() {
        _timeOfDay7= selectedTime;
      });
  ...
  String getTimeDisplayString() {
    return _timeOfDay7.format(context).toString();
  }
  void saveAzkarSwitcherToSharedPreferences()  {
    // await prefs.setBool('Azan switcher', isAzanSwitched);

    prefs.setBool('Azkar switcher${widget.id}', isSwitched7);
  ...
  Future<void> saveTimeToPreferences() async{
    ...
    prefs.setInt('notificationHour${widget.id}', _timeOfDay7.hour);
    prefs.setInt('notificationMinute${widget.id}', _timeOfDay7.minute);
  ...
  Future<void> scheduleNotification() async {
    ...
    var selectedTime =DateTime(now.year,now.month,now.day,_timeOfDay7.hour,_timeOfDay7.minute);
