...
class EveningAzkarPage extends StatefulWidget {
  EveningAzkarPage({Key? key, required this.categoryName, required this.id}) : super(key: key);
...
class _EveningAzkarPageState extends State<EveningAzkarPage> {
  ...
  late TimeOfDay _timeOfDay1=const TimeOfDay(hour: 7, minute: 00);
  ...
  Future<void> initializePreferences() async {
    ...
    setState(() {
      ...
      _timeOfDay1= getTimeFromPreferences() ?? const TimeOfDay(hour: 7, minute: 00);
      isSwitched1 = prefs.getBool('Azkar switcher${widget.id}') ?? false;
      //}
    });
  ...
  String getTimeDisplayString() {
    return _timeOfDay1.format(context).toString();
  }
  void saveAzkarSwitcherToSharedPreferences()  {
    // await prefs.setBool('Azan switcher', isAzanSwitched);

    prefs.setBool('Azkar switcher${widget.id}', isSwitched1);
  ...
  Future<void> saveTimeToPreferences() async{
    ...
    prefs.setInt('notificationHour${widget.id}', _timeOfDay1.hour);
    prefs.setInt('notificationMinute${widget.id}', _timeOfDay1.minute);
  ...
  Future<void> scheduleNotification() async {
    ...
    var selectedTime =DateTime(now.year,now.month,now.day,_timeOfDay1.hour,_timeOfDay1.minute);
