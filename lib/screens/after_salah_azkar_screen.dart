...
class AfterSalahAzkarPage extends StatefulWidget {
  AfterSalahAzkarPage({Key? key, required this.categoryName, required this.id}) : super(key: key);
  final String categoryName;
  final int id;
  var now=DateTime.now();
  @override
  State<AfterSalahAzkarPage> createState() => _AfterSalahAzkarPageState();
}

class _AfterSalahAzkarPageState extends State<AfterSalahAzkarPage> {
  ...
  String getTimeDisplayString() {
    return _timeOfDay5.format(context).toString();
  }
  void saveAzkarSwitcherToSharedPreferences()  {
    // await prefs.setBool('Azan switcher', isAzanSwitched);

    prefs.setBool('Azkar switcher${widget.id}', isSwitched5);
  ...
  Future<void> saveTimeToPreferences() async{
    ...
    prefs.setInt('notificationHour${widget.id}', _timeOfDay5.hour);
    prefs.setInt('notificationMinute${widget.id}', _timeOfDay5.minute);
  ...
  Future<void> scheduleNotification() async {
    ...
    var selectedTime =DateTime(now.year,now.month,now.day,_timeOfDay5.hour,_timeOfDay5.minute);
