...
class WakeUpAzkarPage extends StatefulWidget {
  WakeUpAzkarPage({Key? key, required this.categoryName, required this.id}) : super(key: key);
...
class _WakeUpAzkarPageState extends State<WakeUpAzkarPage> {
  ...
  late TimeOfDay _timeOfDay3=const TimeOfDay(hour: 7, minute: 00);
  ...
  Future<void> initializePreferences() async {
    ...
    setState(() {
      ...
      _timeOfDay3= getTimeFromPreferences() ?? const TimeOfDay(hour: 7, minute: 00);
      isSwitched3 = prefs.getBool('Azkar switcher${widget.id}') ?? false;
      //}
    });
  ...
  String getTimeDisplayString() {
    return _timeOfDay3.format(context).toString();
  }
  void saveAzkarSwitcherToSharedPreferences()  {
    // await prefs.setBool('Azan switcher', isAzanSwitched);

    prefs.setBool('Azkar switcher${widget.id}', isSwitched3);
  ...
  Future<void> saveTimeToPreferences() async{
    ...
    prefs.setInt('notificationHour${widget.id}', _timeOfDay3.hour);
    prefs.setInt('notificationMinute${widget.id}', _timeOfDay3.minute);
  ...
  Future<void> scheduleNotification() async {
    ...
    var selectedTime =DateTime(now.year,now.month,now.day,_timeOfDay3.hour,_timeOfDay3.minute);
